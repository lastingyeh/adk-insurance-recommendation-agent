...
*   `LIVE_MODEL_NAME`: WebSocket 即時對話使用的模型。建議「留空」，由程式依 `GOOGLE_GENAI_USE_VERTEXAI` 自動選對應後端的模型：Vertex 模式為 `gemini-live-2.5-flash-native-audio`、Developer API（key）模式為 `gemini-2.5-flash-native-audio-latest`；要釘特定版本時才填值。
