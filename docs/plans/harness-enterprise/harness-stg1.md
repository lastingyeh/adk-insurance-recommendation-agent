# AI Harness 企業級重構計畫 - 階段一：完整架構、實作與重構計畫書 (Consolidated Stage 1 Plan)

本文件整合了「AI Harness 企業級重構計畫 - 階段一」的所有核心規劃、技術架構、安全護欄設計與 Skill/Prompt 職責分離重構方案。

---

## 導言與背景 (Introduction & Background)

在建構生產級的 AI 代理人（Agent）時，企業面臨兩大核心挑戰：
1. **安全與合規：** 必須在不影響業務邏輯的前提下，提供嚴格的輸入/輸出安全護欄，防止提示詞注入攻擊（Prompt Injection）、個資外流（PII）以及不合規的幻覺輸出。
2. **提示詞膨脹與維護：** 隨著業務場景變複雜，將所有業務規則、商品推薦邏輯、工作流控制全部塞入主 Agent 的系統提示詞中，會導致模型注意力分散、Token 消耗劇增以及推理延遲爆炸。

為了徹底解決上述問題，階段一計畫專注於兩大核心重構：
*   **外置語意安全護欄（Semantic Guardrails）：** 結合 AOP（Aspect-Oriented Programming）哲學，利用 Google ADK 原生的 `BasePlugin` 機制，打造零侵入性、極速決策的安全護欄。
*   **ADK Skill 與 Prompt 職責分離（Separation of Concerns）：** 將主 Agent 的核心「路由與控制」提示詞與特定業務的「領域專家 Knowledge/Skill」完全解耦，實現執行期的漸進式揭露（Progressive Disclosure）。

---

## 第一部分：外置 AOP 安全護欄與流程控制架構設計
...
## 第二部分：ADK 原生 `BasePlugin` 插件整合與異常處置
...
#### 2.1. `app/security/semantic_guardrail.py` 核心架構
...
#### 2.3. `app/services/agent_run_service.py` 異常處理與 SSE 傳輸
...
## 第三部分：安全護欄代理人 (Security Guardrail Agent) 設計
...
## 第四部分：ADK Skill 與 Prompt 職責分離重構
...
### 2. 職責分離 (Separation of Concerns) 設計方案

#### 2.1. 主 Agent 提示詞 (`app/prompts/insurance_agent_prompt.txt`)
...
#### 2.2. 專家 Skill 定義 (`app/skills/insurance-agent-skill/SKILL.md`)
...
## 第五部分：測試、驗證與未來路徑 (Testing, Validation & Roadmap)
...
### 2. 未來路徑 (Roadmap)
在安全地完成了語意護欄插件的實作、並對 Skill/Prompt 進行職責分離重構後。我們下一步將依照 `harness-enterprise-overhaul.md` 的藍圖推進：
*   **階段二：** 多代理人 A2A（Agent-to-Agent）協作。
*   **階段三：** 基於 pgvector 的長期語意記憶機制。
*   **階段四：** 即時語音 WebSocket 與 Live Stream 整合。
